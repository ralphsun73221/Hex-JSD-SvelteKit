...
- 上個星期再處理完樣式推送 clock 的時候發現一個情況，就是點擊 Nav 的時鐘一樣會出現九九乘法表，我猜測的情況可能是由於 Compile 的時候並不會分開去處理不同頁面的 JS，而是統一由單一的 JS 來去處理，由於我使用的方式並不是官方建議的做法（因為官方建議用 SveletKit），所以才會導致無論如何都只會吐九九乘法表的內容
